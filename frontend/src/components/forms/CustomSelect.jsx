...
export default function CustomSelect({
  ...
  // Helper to find selected option (flat or nested in groups)
  const findSelectedOption = (opts, val) => {
    for (const opt of opts) {
      if (opt.isGroup && opt.options) {
        const found = opt.options.find(sub => String(sub.value) === String(val));
            ...
            if (opt.isGroup) {
              return (
                <div key={i} className="mb-2 last:mb-0">
                  <div className="px-4 py-1.5 text-xs font-bold text-slate-400 uppercase tracking-wider bg-slate-50/50">
                    {opt.label}
                  </div>
